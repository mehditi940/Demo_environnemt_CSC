...
const ModelLoader = forwardRef(
  ({ modelPath, onPartsLoaded, partSettings, drawMode, enable }, ref) => {
    ...
    // Load 3MF model with error handling
    const rawModel = useLoader(ThreeMFLoader, modelPath, (loader) => {
      loader.setPath('');
      return loader;
    });
    ...
    useEffect(() => {
      ...
      try {
        console.log("Processing 3MF model:", rawModel);
        ...
        console.error("Error processing 3MF model:", error);
        ...
        fallbackMesh.name = "Fallback Model - 3MF Error";
        
        scene.add(fallbackMesh);
        modelRef.current = fallbackMesh;
        
        const fallbackParts = [{ name: "Fallback Model - 3MF Error", object: fallbackMesh }];
    ...
    return (
      <>
        {loadingError && <mesh position={[0, 0, 0]}><boxGeometry args={[1, 1, 1]} /><meshBasicMaterial color="red" /></mesh>}
