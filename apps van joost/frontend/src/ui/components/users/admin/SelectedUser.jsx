...
import { handleUpdateUser, handleDeleteUser } from '../../../../business/controller/userController';
...
const SelectedUser = ({ user, onPatientDeleted }) => {
  ...
  const confirmSubmit = async () => {
     try {
      // Use handleDeleteUser to completely remove the user from the database
      ...
      if (result.success) {
        ...
        showNotification("Gebruiker is volledig verwijderd uit de database!");
      } else {
  ...
  return (
    <div className='selected-room-card'>
      <div className="room-header">
        <h2 className="room-title">{user.email}</h2>
      </div>
      ...
      <div className="room-details">
        <div className="detail-section">
          ...
          <div className="detail-item">
            ...
            <div className="detail-value">{new Date(user.updatedAt).toLocaleDateString('nl-NL', {
              year: 'numeric',
              month: 'long',
              day: 'numeric',
              hour: '2-digit',
      ...
      <div className="room-actions">
        ...
        <button className="btn btn-danger" onClick={() => setShowBevestigingPopup(true)}>
          ...
          <span>Gebruiker deactiveren</span>
        </button>
      </div>
      ...
      {showBevestigingPopup && (
        <div className="modal-overlay">
          <div className="modal-content">
            <p>Weet je zeker dat je <strong>{user.email}</strong> wilt verwijderen?</p>
