...
import { Patient, patientInsertSchema } from "../../schemas/patient";
 ...
 *       description: Allows surgeon to create a new patient in the database
 *       tags: [Patient]
 *       security:
 *         - bearerAuth: []
 *       requestBody:
 ...
 *       description: Allows surgeon to get a list of patients with limit
 *       tags: [Patient]
 *       security:
 *         - bearerAuth: []
 *       parameters:
 ...
 *       description: Allows surgeon to update a patient's information
 *       tags: [Patient]
 *       security:
 *         - bearerAuth: []
 *       parameters:
 ...
 *       description: Allows a surgeon to delete a patient.
 *       tags: [Patient]
 *       security:
 *         - bearerAuth: []
 *       parameters:
 ...
 *       description: Allows a surgeon to get a patient.
 *       tags: [Patient]
 *       security:
 *         - bearerAuth: []
 *       parameters:
    ...
    try {
      try {
        ...
        const existingPatient = await db.patient.findUnique({
          where: { nummer: parsedBody.nummer }
        });
        ...
        const patient = await db.patient.create({
          data: {
            ...parsedBody,
            createdAt: parsedBody.createdAt ? new Date(parsedBody.createdAt) : undefined,
            updatedAt: parsedBody.updatedAt ? new Date(parsedBody.updatedAt) : undefined
          }
        });
...
patientRouter.put(
  ...
  async (req, res) => {
    try {
      ...
      const patientResult = await db.patient.findUnique({
        where: { id: patientId }
      });
      ...
      const updatedPatient = await db.patient.update({
        ...
        data: {
          ...
          updatedAt: updateData.updatedAt ? new Date(updateData.updatedAt) : undefined
        }
      });
